...
describe('EnhancedShihTzu', () => {
  ...
  describe('Thought Bubble Positioning', () => {
    it('positions thought bubble above companion', () => {
      ...
      const thoughtBubble = container.querySelector('.animate-float-subtle')?.parentElement as HTMLElement;
  ...
  describe('Variant Rendering', () => {
    it('renders balloon variant decorations', () => {
      const { container } = render(
        <EnhancedShihTzu variant="balloon" />
      );

      // Should have balloon circles
      const balloons = container.querySelectorAll('circle[fill*="url(#balloonGradient"]');
      expect(balloons.length).toBeGreaterThan(0);
    });

    it('applies correct colors for different variants', () => {
      const variants = ['default', 'winter', 'party', 'workout', 'balloon'] as const;
      
      variants.forEach(variant => {
        const { container } = render(
          <EnhancedShihTzu variant={variant} />
        );
