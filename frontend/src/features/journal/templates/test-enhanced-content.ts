...
export function testEnhancedContentFormat() {
  ...
  const sectionResponses: SectionResponse[] = [
    ...
    },
    {
      sectionId: "lessons",
      value:
        "Taking breaks helps maintain focus. Sometimes stepping away from a problem brings clarity.",
  ...
  sectionResponses.forEach((original, index) => {
    ...
    const matches =
      JSON.stringify(original.value) === JSON.stringify(parsed.value);
    console.log(
      `Section ${original.sectionId}: ${matches ? "✓ Match" : "✗ Mismatch"}`,
    );
    if (!matches) {
      allMatch = false;
...
Struggled with a complex bug in the authentication system, but finally found the root cause.`;
  ...
  const legacyParsed = enhancedJournalContentUtils.contentToSections(
    template,
    legacyContent,
  );
  ...
  legacyParsed.forEach((section) => {
    ...
    console.log(
      `- ${sectionDef?.title}: ${typeof section.value === "string" ? section.value.substring(0, 50) + "..." : section.value}`,
    );
  });
}
