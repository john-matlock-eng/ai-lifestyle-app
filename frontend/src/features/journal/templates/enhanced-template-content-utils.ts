...
console.log(
  "%c[ContentUtils v2] MODULE LOADED - NEW VERSION WITHOUT DIV WRAPPER",
  "background: #4CAF50; color: white; padding: 4px 8px; border-radius: 4px; font-weight: bold",
);
console.log(
  '[ContentUtils v2] This version saves text content WITHOUT <div class="section-content"> wrapper',
);

// Global version check function for debugging
if (typeof window !== "undefined") {
  (
    window as unknown as { checkJournalVersion: () => void }
  ).checkJournalVersion = () => {
  ...
  console.log(
    "[ContentUtils v2] Run checkJournalVersion() in console to verify version",
  );
}
...
export const enhancedJournalContentUtils = {
  ...
  sectionsToContent(
    ...
    log("Converting sections to content", {
    ...
    const htmlSections = sections
      .map((section) => {
        ...
        if (!sectionDef) {
          log(
            `Warning: Section definition not found for ID: ${section.sectionId}`,
          );
          return "";
        }
        ...
        const metadata = {
          ...
          completedAt:
            section.metadata?.completedAt || new Date().toISOString(),
          ...(section.metadata || {}),
        };
        ...
        switch (sectionDef.type) {
          ...
          case "mood": {
            const mood = sectionDef.options?.moods?.find(
              (m) => m.value === section.value,
            );
          ...
          case "choice": {
            const choice = sectionDef.options?.choices?.find(
              (c) => c.value === section.value,
            );
          ...
          case "checklist": {
            ...
            const checklistHtml = Object.entries(items)
              .map(
                ([item, checked]) => `<li>${checked ? "☑" : "☐"} ${item}</li>`,
              )
              .join("\n");
            if (checklistHtml) {
  ...
  contentToSections(
    ...
    log("Parsing content to sections", {
      template: template.name,
...
function extractSectionValue(
  ...
  switch (sectionType) {
    case "text": {
      ...
      const divMatch = contentWithoutTitle.match(
      ...
      if (divMatch) {
        ...
        const anyDivMatch = contentWithoutTitle.match(
          /<div[^>]*>([\s\S]*?)<\/div>/i,
        );
        if (anyDivMatch) {
