...
<!--/SECTION-->
      ...
      const sections = enhancedJournalContentUtils.contentToSections(mockTemplate, content);
    ...
    it('should handle plain text content without markers', () => {
      ...
      expect(sections).toHaveLength(3);
      expect(sections[0].sectionId).toBe('text-section');
      expect(sections[0].value).toBe('This is plain text content without any markers.');
...
<!--/SECTION-->`;
      ...
      expect(sections[0].value).toBe('Testing & special chars: <tag> "quotes" \'apostrophe\'');
    });
...
<!--/SECTION-->`;
    ...
    it('should handle empty content', () => {
      const sections = enhancedJournalContentUtils.contentToSections(mockTemplate, '');
  ...
  describe('roundtrip conversion', () => {
    it('should preserve content through conversion cycle', () => {
      ...
      // Convert to content
      const content = enhancedJournalContentUtils.sectionsToContent(mockTemplate, originalSections);
      
      // Parse back to sections
      const parsedSections = enhancedJournalContentUtils.contentToSections(mockTemplate, content);
