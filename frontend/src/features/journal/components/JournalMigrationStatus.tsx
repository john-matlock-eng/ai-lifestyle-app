...
import {
  useJournalMigration,
  JournalContentMigrationService,
} from "../services/journal-content-migration";
...
export const JournalMigrationStatus: React.FC = () => {
  const { migrateEntries, isRunning, progress, result, reset } =
    useJournalMigration();
  ...
  const handleMigrate = async () => {
    ...
    const migrationResult = await migrateEntries(
      needsMigration as JournalEntry[],
    );
  ...
  const getStatusMessage = () => {
    if (isRunning)
      return `Migrating entries... (${progress.current}/${progress.total})`;
