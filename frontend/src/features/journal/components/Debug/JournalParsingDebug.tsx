...
export const JournalParsingDebug: React.FC = () => {
  ...
  const testParsing = () => {
    try {
      ...
      const sections = enhancedJournalContentUtils.contentToSections(template, inputContent);
  ...
  const loadSampleContent = (type: 'with-markers' | 'plain-text' | 'legacy') => {
    const samples = {
...
Rating: 8/10`
  ...
  return (
    <div className="p-4 space-y-4 bg-gray-50 rounded-lg">
      ...
      <div>
        ...
        >
          {Object.keys(enhancedTemplates).map(key => (
            <option key={key} value={key}>{enhancedTemplates[key as keyof typeof enhancedTemplates].name}</option>
          ))}
        </select>
      </div>
      ...
      {/* Input Content */}
      <div>
        <label className="block text-sm font-medium mb-2">Input Content (HTML)</label>
        <textarea
          value={inputContent}
      ...
      {convertedContent && (
        <div>
          ...
          <div className="p-3 bg-white border rounded">
            <pre className="whitespace-pre-wrap font-mono text-sm">{convertedContent}</pre>
          </div>
        </div>
      )}
