...
const EmotionWheel: React.FC<EmotionWheelProps> = ({
  ...
  const safeSelectedEmotions = Array.isArray(selectedEmotions)
    ? selectedEmotions
    : [];
  ...
  const middleRadius = wheelSize * 0.3; // Slightly larger
        ...
        >
          ...
          >
            ...
            {coreEmotions.map((coreEmotion, coreIndex) => {
              ...
              const secondaryAngleSize =
                coreAngleSize / secondaryEmotions.length;

              return secondaryEmotions.map((emotion, secIndex) => {
                const startAngle =
                  coreStartAngle + secIndex * secondaryAngleSize;
            ...
            {coreEmotions.map((coreEmotion, coreIndex) => {
              ...
              return secondaryEmotions.map((secEmotion, secIndex) => {
                ...
                if (
                  progressiveReveal &&
                  focusedSecondaryEmotion !== secEmotion.id
                ) {
                  return null;
                }
                ...
                const secStartAngle =
                  coreStartAngle + secIndex * secondaryAngleSize;
                const tertiaryAngleSize =
                  secondaryAngleSize / tertiaryEmotions.length;

                return tertiaryEmotions.map((emotion, terIndex) => {
                  const startAngle =
                    secStartAngle + terIndex * tertiaryAngleSize;
      ...
      {tooltip &&
        ...
        (() => {
          ...
          if (left < padding) {
            ...
            left = Math.max(
              padding,
              Math.min(left, containerWidth - tooltipWidth - padding),
            );
          }
          ...
          if (top + tooltipHeight > containerHeight - padding) {
            ...
            top = Math.min(
              tooltip.y - tooltipHeight - offset,
          ...
          top = Math.max(
            padding,
            Math.min(top, containerHeight - tooltipHeight - padding),
          );
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-lg">
                  {getEmotionEmoji(currentEmotion.id)}
                </span>
                <span className="font-medium">{currentEmotion.label}</span>
              </div>
              <div className="text-xs text-muted mt-1">
                Click to{" "}
                {safeSelectedEmotions.includes(currentEmotion.id)
                  ? "deselect"
                  : "select"}
              </div>
            </div>
        ...
        <div className="mt-4 p-3 bg-surface rounded-lg border border-surface-muted">
          <div className="flex items-center gap-2 text-sm">
            ...
            <div className="flex items-center gap-1">
              ...
              <span
                className={`px-2 py-1 rounded ${focusedSecondaryEmotion ? "bg-accent/20 text-accent font-medium" : "bg-muted/20 text-muted"}`}
              >
                2. Secondary
              </span>
              <span className="text-muted">→</span>
              <span
                className={`px-2 py-1 rounded ${showCompleteButton ? "bg-accent/20 text-accent font-medium" : "bg-muted/20 text-muted"}`}
              >
                3. Specific
              </span>
            </div>
            ...
            {safeSelectedEmotions
              .filter((id) => {
                ...
                return (
                  ...
                  emotion.level !== "core" &&
                  (!emotion.parent ||
                    !safeSelectedEmotions.includes(emotion.parent))
                );
              })
