...
export const JournalEntryRenderer: React.FC<JournalEntryRendererProps> = ({
  ...
  const parseSectionMarkers = (html: string): ParsedSection[] => {
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      try {
        ...
        console.log("[parseSectionMarkers] Parsed section:", {
          sectionId,
          type,
          contentLength:
  ...
  const extractSectionContent = (html: string, type: string): unknown => {
    ...
    switch (type) {
      case "emotions": {
        ...
        const emotionElements = tempDiv.querySelectorAll(
          "li, span.emotion-item",
        );
        if (emotionElements.length > 0) {
          return Array.from(emotionElements)
      ...
      case "goals": {
        ...
        const goalLinks = tempDiv.querySelectorAll(
          'a[href*="goal"], .goal-link',
        );
      ...
      case "choice": {
        ...
        const selectedChoice = tempDiv.querySelector(
    ...
    h3Elements.forEach((h3) => {
      ...
      while (nextNode) {
        if (
          ...
          (nextNode as Element).tagName === "H3"
        ) {
          break;
        }
        if (
          nextNode.nodeType === Node.COMMENT_NODE &&
          nextNode.textContent?.trim() === "/SECTION"
        ) {
          break;
        }
    ...
    switch (section.type) {
      ...
      case "checklist":
        return renderChecklistSection(
          section.content as Record<string, boolean>,
          sectionDef,
        );
    ...
    return (
      <ReactMarkdown
        components={{
          ...
          ul: ({ children }) => (
            <ul className="mb-4 space-y-2 ml-6 list-disc">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="mb-4 space-y-2 ml-6 list-decimal">{children}</ol>
          ),
        ...
        {checklistItems.map((item) => {
          ...
          return (
            <li key={item.id} className="flex items-start gap-3">
              <span
                className={`text-lg ${isChecked ? "text-green-500" : "text-gray-400"}`}
              >
                {isChecked ? "✅" : "⬜"}
              </span>
              <span
                className={`flex-1 ${isChecked ? "line-through text-muted" : ""}`}
              >
                {item.label}
              </span>
            </li>
  ...
  const renderChoiceSection = (
    ...
    const choiceOption = sectionDef.options?.choices?.find(
      (c) => c.value === choice || c.label === choice,
    );

    return (
      <div className="inline-flex items-center px-4 py-2 bg-accent/10 text-accent rounded-full">
        {choiceOption?.icon && (
          <span className="mr-2">{choiceOption.icon}</span>
        )}
    ...
    return (
      <div className="space-y-8">
        <section className="journal-section">
          ...
          <div className="emotion-display">
            {reflectionData.emotions ? (
              renderMarkdownText(reflectionData.emotions)
            ) : (
              <span className="journal-empty-content">
        ...
        <section className="journal-section">
          ...
          {Array.isArray(reflectionData.gratitude) &&
          reflectionData.gratitude.length > 0 ? (
            <ul className="space-y-3">
          ...
          ) : typeof reflectionData.gratitude === "string" ? (
            renderMarkdownText(reflectionData.gratitude)
          ) : (
            <span className="journal-empty-content">
        ...
        <section className="journal-section">
          ...
          {Array.isArray(reflectionData.highlights) &&
          reflectionData.highlights.length > 0 ? (
            <ul className="space-y-3">
          ...
          ) : typeof reflectionData.highlights === "string" ? (
            renderMarkdownText(reflectionData.highlights)
          ) : (
            <span className="journal-empty-content">No highlights added</span>
          )}
        </section>
  ...
  const parseContent = (content: string) => {
    ...
    if (
      (content.includes("<h3>") || content.includes("<H3>")) &&
      content.includes("</h3>")
    ) {
      ...
      <div className={`journal-content ${className}`}>
        <ReactMarkdown
          components={{
            ...
            h4: ({ children }) => (
              <h4 className="text-lg font-medium mt-3 mb-2">{children}</h4>
            ),
            p: ({ children }) => (
              <p className="mb-4 leading-relaxed">{children}</p>
            ),
            ul: ({ children }) => (
              <ul className="mb-4 space-y-2 ml-6">{children}</ul>
            ),
            ol: ({ children }) => (
              <ol className="mb-4 space-y-2 ml-6 list-decimal">{children}</ol>
            ),
  ...
  return (
    <div className={`journal-content ${className}`}>{renderedContent}</div>
  );
};
