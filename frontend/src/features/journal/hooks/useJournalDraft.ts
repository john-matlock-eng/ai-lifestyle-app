...
export function useJournalDraft({
    ...
    try {
      ...
      try {
        ...
        if (e instanceof Error && e.name === 'QuotaExceededError') {
          console.warn('[useJournalDraft] Storage quota exceeded, cleaning old drafts...');
          ...
          try {
            ...
            console.error('[useJournalDraft] Still cannot save draft after cleanup:', e2);
            throw e2;
          }
        } else {
          throw e;
      ...
      const sortedDrafts = draftsList
        .sort((a: { lastSaved: string }, b: { lastSaved: string }) => new Date(b.lastSaved).getTime() - new Date(a.lastSaved).getTime())
        .slice(0, 50);
  ...
  const cleanOldDrafts = useCallback((daysOld = 30) => {
    try {
      ...
      localStorage.setItem(DRAFT_LIST_KEY, JSON.stringify(filtered));
      console.log(`[useJournalDraft] Cleaned ${draftsList.length - filtered.length} old drafts`);
    } catch (error) {
