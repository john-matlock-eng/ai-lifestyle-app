...
export const useHabits = () => {
  ...
  const { data: todayData, isLoading: isLoadingHabits } = useQuery({
  ...
  >({
    mutationFn: async ({ habitId, completed, note }) => {
      ...
      if (todayData) {
        ...
        // Update stats
        if (todayData) {
          const updatedHabits = queryClient.getQueryData<{
            habits: Habit[];
            stats: UserStats;
          }>(["habits", "today"]);
          const habitsCompletedToday =
            updatedHabits?.habits.filter((h) => h.completedToday).length ?? 0;
          ...
          queryClient.setQueryData(["habits", "today"], {
            ...
            stats: {
              ...updatedHabits?.stats,
              totalPoints:
                (updatedHabits?.stats?.totalPoints ?? 0) + data.points,
              totalCheckIns:
                (updatedHabits?.stats?.totalCheckIns ?? 0) +
                (completed ? 1 : 0),
              habitsCompletedToday,
              // Recalculate level progress
              nextLevelProgress:
                ((((updatedHabits?.stats?.totalPoints ?? 0) + data.points) %
                  ((updatedHabits?.stats?.currentLevel ?? 1) * 100)) /
                  ((updatedHabits?.stats?.currentLevel ?? 1) * 100)) *
                100,
              currentLevel:
                Math.floor(
    ...
    onSuccess: (_, habitId) => {
      ...
      if (todayData) {
        queryClient.setQueryData(["habits", "today"], {
          ...todayData,
          habits: todayData.habits.map((habit) =>
            habit.id === habitId ? { ...habit, skippedToday: true } : habit,
          ),
        });
      }
  ...
  const createMutation = useMutation({
    mutationFn: (habitData: CreateHabitRequest) =>
  ...
  const checkInHabit = useCallback(
    async (habitId: string, completed: boolean, note?: string) => {
      try {
