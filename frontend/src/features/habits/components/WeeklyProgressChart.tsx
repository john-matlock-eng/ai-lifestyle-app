...
export const WeeklyProgressChart: React.FC<WeeklyProgressChartProps> = ({
  ...
  const dailyCompletions = days.map((_, index) => {
    ...
    return totalHabitsForDay > 0
      ? (completedForDay / totalHabitsForDay) * 100
      : 0;
  });
        ...
        {days.map((day, index) => {
          ...
          return (
            <div key={day} className="flex items-center space-x-3">
              <span
                className={`text-xs w-8 ${isToday ? "text-accent font-bold" : "text-muted"}`}
              >
                {day}
              </span>
              <div className="flex-1 h-2 bg-surface-muted rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-500 ease-out ${
                    isToday
                      ? "bg-gradient-to-r from-accent to-accent-hover glow"
