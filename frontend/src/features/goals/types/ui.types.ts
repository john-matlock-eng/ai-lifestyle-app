...
export const GOAL_PATTERNS: Record<GoalPattern, GoalPatternConfig> = {
  ...
  target: {
    ...
    examples: ['Lose 20 lbs by June', 'Finish project by Q2', 'Learn Spanish B2 level'],
  ...
  streak: {
    ...
    examples: ['100 day code streak', 'No smoking 30 days', 'Daily journal entries'],
...
export const METRIC_OPTIONS: Array<{ value: Metric; label: string; units: string[] }> = [
  { value: 'count', label: 'Count', units: ['times', 'reps', 'pages', 'items', 'tasks'] },
  { value: 'duration', label: 'Duration', units: ['minutes', 'hours', 'seconds'] },
  { value: 'amount', label: 'Amount', units: ['liters', 'gallons', 'cups', 'glasses'] },
  { value: 'weight', label: 'Weight', units: ['lbs', 'kg', 'grams'] },
  { value: 'distance', label: 'Distance', units: ['miles', 'km', 'meters', 'steps'] },
...
export function getDefaultsByPattern(pattern: GoalPattern): {
  ...
  const defaults = {
    recurring: { period: 'day' as Period, targetType: 'minimum' as TargetType, direction: 'increase' as Direction },
    milestone: { targetType: 'exact' as TargetType, direction: 'increase' as Direction },
    target: { targetType: 'exact' as TargetType, direction: 'increase' as Direction },
    streak: { targetType: 'minimum' as TargetType, direction: 'increase' as Direction, period: 'day' as Period },
    limit: { targetType: 'maximum' as TargetType, direction: 'maintain' as Direction, period: 'day' as Period },
  };
...
export function formatGoalValue(value: number, unit: string): string {
  // Handle plural forms
  if (value !== 1 && !unit.endsWith('s') && ['page', 'rep', 'item', 'task', 'time'].includes(unit)) {
    return `${value} ${unit}s`;
  }
  return `${value} ${unit}`;
}
