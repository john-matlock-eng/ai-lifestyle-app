...
export const SharedJournalsPage: React.FC = () => {
  ...
  const { data: sharedJournals, isLoading, refetch } = useQuery({
  ...
  const getShareStatus = (shareInfo: SharedJournal['shareInfo']) => {
    if (shareInfo.expiresAt) {
      ...
      const hoursUntilExpiry = (expiresDate.getTime() - Date.now()) / (1000 * 60 * 60);
  ...
  const getPermissionBadges = (permissions: string[]) => {
    const badges = permissions.map(permission => {
      ...
      return labels[permission] || { text: permission, color: 'bg-gray-100 text-gray-700' };
    });
    return badges;
  };
      ...
      <div className="container mx-auto py-8 px-4 max-w-6xl">
        ...
        {filteredJournals.length === 0 ? (
          <div className="text-center py-12 glass rounded-2xl">
            ...
            <h3 className="text-lg font-medium text-theme mb-2">No shared journals</h3>
          ...
          <div className="grid gap-4">
            {(filter === 'all' || filter === 'shared-by-me') && sharedByMe.length > 0 && (
              <>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-xs text-muted mb-2">
                                ...
                                <span className="flex items-center gap-1">
                                  ...
                                  Shared {formatDistanceToNow(new Date(shareInfo.sharedAt), { addSuffix: true })}
                                </span>
                                <span className="flex items-center gap-1">
            ...
            {(filter === 'all' || filter === 'shared-with-me') && sharedWithMe.length > 0 && (
              <>
                ...
                {sharedWithMe.map(({ entry, shareInfo }) => {
                  const status = getShareStatus(shareInfo);
                  const permissionBadges = getPermissionBadges(shareInfo.permissions);
                        ...
                        <div className="flex-1">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-xs text-muted mb-2">
                                <span>Shared by {shareInfo.sharedBy || 'Unknown'}</span>
                                <span className="flex items-center gap-1">
                                  <Clock className="w-3 h-3" />
        ...
        <div className="mt-8 p-6 bg-accent/10 rounded-xl">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-theme mb-2">End-to-End Encryption</h3>
              <p className="text-sm text-muted">
                All shared journals remain encrypted. Only authorized recipients with the correct 
                decryption keys can access the content. Share access can be revoked at any time.
              </p>
            </div>
          </div>
        </div>
