    ...
    <div className="min-h-screen relative">
      ...
      {theme === "balloon" && (
        <div className="absolute inset-0 -z-10 overflow-hidden">
          ...
          <div
            className="absolute top-0 right-20 h-72 w-72 rounded-full bg-[#ec4899] opacity-25 blur-2xl animate-float-orb"
            style={{ animationDelay: "2s" }}
          />
          <div
            className="absolute top-40 left-1/3 h-56 w-56 rounded-full bg-[#06b6d4] opacity-20 blur-2xl animate-float-orb"
            style={{ animationDelay: "4s" }}
          />

          {/* Medium balloons */}
          <div
            className="absolute top-20 right-1/3 h-48 w-48 rounded-full bg-[#f9a8d4] opacity-25 blur-xl animate-float-orb"
            style={{ animationDelay: "1s" }}
          />
          <div
            className="absolute top-60 left-20 h-40 w-40 rounded-full bg-[#8b5cf6] opacity-20 blur-xl animate-float-orb"
            style={{ animationDelay: "3s" }}
          />
          <div
            className="absolute top-32 right-10 h-44 w-44 rounded-full bg-[#ec4899] opacity-25 blur-xl animate-float-orb"
            style={{ animationDelay: "5s" }}
          />

          {/* Small accent balloons */}
          <div
            className="absolute top-5 left-1/2 h-32 w-32 rounded-full bg-[#06b6d4] opacity-30 blur-lg animate-float-orb"
            style={{ animationDelay: "0.5s" }}
          />
          <div
            className="absolute top-48 right-1/4 h-28 w-28 rounded-full bg-[#f9a8d4] opacity-30 blur-lg animate-float-orb"
            style={{ animationDelay: "2.5s" }}
          />
          <div
            className="absolute top-80 left-1/4 h-36 w-36 rounded-full bg-[#8b5cf6] opacity-25 blur-lg animate-float-orb"
            style={{ animationDelay: "4.5s" }}
          />

          {/* Bottom layer balloons */}
          <div
            className="absolute bottom-20 left-10 h-56 w-56 rounded-full bg-[#ec4899] opacity-20 blur-2xl animate-float-orb"
            style={{ animationDelay: "1.5s" }}
          />
          <div
            className="absolute bottom-0 right-1/3 h-64 w-64 rounded-full bg-[#06b6d4] opacity-25 blur-2xl animate-float-orb"
            style={{ animationDelay: "3.5s" }}
          />
          <div
            className="absolute bottom-40 left-1/2 h-48 w-48 rounded-full bg-[#f9a8d4] opacity-20 blur-xl animate-float-orb"
      ...
      {theme !== "balloon" && (
        <div className="absolute inset-0 -z-10 overflow-hidden">
          <div
            className={`absolute top-0 -left-1/4 h-96 w-96 rounded-full opacity-20 blur-3xl animate-float-orb ${
              isVibrantTheme
                ? "bg-gradient-to-br from-[#ff006e] to-[#8338ec]"
                : isDarkTheme
                  ? "bg-accent/30"
                  : "bg-accent/20"
            }`}
          />
          <div
            className={`absolute top-1/2 -right-1/4 h-96 w-96 rounded-full opacity-20 blur-3xl animate-float-orb ${
              isVibrantTheme
                ? "bg-gradient-to-tl from-[#3a86ff] to-[#ff006e]"
                : isDarkTheme
                  ? "bg-accent/30"
                  : "bg-accent/20"
            }`}
      ...
      <div className="welcome-banner relative overflow-hidden mb-6 -mx-4 -mt-6 sm:-mx-6 lg:-mx-8">
        <div
          className={`absolute inset-0 ${
            theme === "balloon"
              ...
              : isVibrantTheme
                ? "bg-gradient-to-r from-[#ff006e] via-[#8338ec] to-[#3a86ff]"
        ...
        {theme === "balloon" && (
          <>
            ...
            <div
              className="absolute -top-5 left-1/4 w-24 h-24 rounded-full bg-[#06b6d4] opacity-35 blur-lg animate-float-orb"
              style={{ animationDelay: "1s" }}
            />
            <div
              className="absolute -bottom-10 -right-10 w-40 h-40 rounded-full bg-[#8b5cf6] opacity-40 blur-xl animate-float-orb"
              style={{ animationDelay: "2s" }}
            />
            <div
              className="absolute top-1/2 right-1/3 w-28 h-28 rounded-full bg-[#ec4899] opacity-35 blur-lg animate-float-orb"
              style={{ animationDelay: "3s" }}
            />
          </>
        )}
          ...
          <div className="mt-6 bg-black/10 backdrop-blur-sm rounded-lg p-4">
            <div className="flex justify-between text-sm font-medium text-white mb-2">
              ...
              <span className="text-white/95 drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                {Math.round(stats.nextLevelProgress)}% to Level{" "}
                {stats.currentLevel + 1}
              </span>
            </div>
            <div
              className={`h-5 ${theme === "balloon" ? "bg-white/40" : "bg-white/30"} rounded-full overflow-hidden backdrop-blur-sm shadow-inner`}
            >
              <div
                className={`h-full ${
                  theme === "balloon"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div
          className={`relative overflow-hidden rounded-2xl p-5 text-white hover:scale-105 transition-all duration-300 cursor-pointer shadow-xl hover:shadow-2xl group ${
            theme === "balloon"
        ...
        >
          <div className="relative z-10">
            <div className="text-4xl mb-2 transform group-hover:scale-110 transition-transform animate-fire-flicker">
              🔥
            </div>
            <div className="text-3xl font-bold drop-shadow-lg">
