...
import {
  GOAL_PATTERNS,
  GOAL_CATEGORIES,
} from "../../features/goals/types/ui.types";
import {
  listGoals,
  updateGoal,
  archiveGoal,
...
const GoalsPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: [
      "goals",
      { selectedStatuses, selectedPatterns, selectedCategories },
    ],
    queryFn: () =>
      listGoals({
                ...
                <button
                  onClick={() => {
                    if (selectedStatuses.includes("paused")) {
                      setSelectedStatuses(
                        selectedStatuses.filter((s) => s !== "paused"),
                      );
                    } else {
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-surface rounded-lg shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-green-600">
              {
                data.goals.filter((g) => g.progress.percentComplete >= 100)
                  .length
              }
            </div>
          ...
          <div className="bg-surface rounded-lg shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-orange-600">
              {Math.max(
                ...data.goals.map((g) => g.progress.currentStreak || 0),
              )}
            </div>
          ...
          <div className="bg-surface rounded-lg shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-primary-600">
              {Math.round(
                data.goals.reduce((sum, g) => sum + g.progress.successRate, 0) /
                  data.goals.length,
              )}
              %
            </div>
