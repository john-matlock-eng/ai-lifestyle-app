    ...
    try {
      ...
      try {
        ...
        const hasKeyMismatch =
          publicKeyId &&
          localPublicKeyId &&
          publicKeyId.toLowerCase().trim() !==
            localPublicKeyId.toLowerCase().trim();
          ...
          if (axiosError.response?.status === 404) {
            ...
            try {
              ...
              if (profile.encryptionEnabled) {
                ...
                console.log(
                  "[EncryptionMismatch] Profile shows encryption enabled, assuming keys are in sync",
                );
                setState({ hasMismatch: false, isChecking: false });
              } else {
                // Profile says no encryption, but we have local keys
                console.log(
                  "[EncryptionMismatch] Local keys exist but profile shows no encryption",
                );
                setState({
                  hasMismatch: true,
                  isChecking: false,
              ...
              console.log(
                "[EncryptionMismatch] Can't verify encryption status, assuming no mismatch",
              );
              setState({ hasMismatch: false, isChecking: false });
            }
          } else {
            ...
            console.log(
              "[EncryptionMismatch] Server error checking encryption:",
