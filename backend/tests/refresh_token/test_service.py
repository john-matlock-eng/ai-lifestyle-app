...
from refresh_token.models import CognitoTokenResponse, RefreshTokenRequest, RefreshTokenResponse
...
class TestTokenRefreshService:
    ...
    def test_invalid_token_error(self, service, mock_cognito_client):
        ...
        mock_cognito_client.refresh_access_token.side_effect = InvalidTokenError("Invalid token")
    ...
    def test_expired_token_error(self, service, mock_cognito_client):
        ...
        # Setup mock to raise error
        mock_cognito_client.refresh_access_token.side_effect = ExpiredTokenError("Token expired")
    ...
    def test_revoked_token_error(self, service, mock_cognito_client):
        ...
        # Setup mock to raise error
        mock_cognito_client.refresh_access_token.side_effect = RevokedTokenError("Token revoked")
    ...
    def test_refresh_error_passthrough(self, service, mock_cognito_client):
        ...
        # Setup mock to raise error
        mock_cognito_client.refresh_access_token.side_effect = RefreshError("Generic refresh error")
    ...
    def test_unexpected_error_handling(self, service, mock_cognito_client):
        ...
        mock_cognito_client.refresh_access_token.side_effect = RuntimeError("Unexpected error")
    ...
    def test_validate_new_token_success(self, service, mock_cognito_client):
        ...
        mock_cognito_client.get_user_from_access_token.return_value = {
            "username": "test-user",
            "attributes": {"email": "test@example.com", "email_verified": "true"},
        }
        ...
        mock_cognito_client.get_user_from_access_token.assert_called_once_with("valid-access-token")

    def test_validate_new_token_failure(self, service, mock_cognito_client):
        """Test failed token validation."""
        # Setup mock to raise error
        mock_cognito_client.get_user_from_access_token.side_effect = Exception("Invalid token")
    ...
    def test_logging_successful_refresh(self, mock_logger, service, mock_cognito_client):
        ...
        mock_logger.info.assert_any_call(
            "Processing token refresh request", extra={"has_refresh_token": True}
        )
        mock_logger.info.assert_any_call(
            "Token refresh completed successfully",
            extra={"expires_in": 3600, "token_type": "Bearer"},
        )
    ...
    def test_logging_error(self, mock_logger, service, mock_cognito_client):
        ...
        mock_logger.exception.assert_called_once_with(
            "Unexpected error during token refresh", extra={"error": "Error"}
        )
