...
class TestGoalUserJourney:
    ...
    def test_daily_habit_journey(self, api_client, auth_headers):
        ...
        response = api_client.get(f"/v1/goals/{goal_id}/progress?period=current",
                                headers=auth_headers)
        ...
        response = api_client.get(f"/v1/goals/{goal_id}/activities?startDate=2024-01-20&endDate=2024-01-22",
                                headers=auth_headers)
        assert response.status_code == 200
        ...
        response = api_client.get("/v1/goals?status=active&goalPattern=recurring",
                                headers=auth_headers)
        assert response.status_code == 200
        goals = response.json()
    ...
    def test_limit_goal_journey(self, api_client, auth_headers):
        ...
        assert progress["statistics"]["averageValue"] == sum(u["minutes"] for u in daily_usage) / len(daily_usage)
    ...
    def test_goal_archival_and_completion(self, api_client, auth_headers):
        ...
        response = api_client.get("/v1/goals?status=active",
                                headers=auth_headers)
        assert response.status_code == 200
        goals = response.json()
        ...
        response = api_client.get("/v1/goals?status=archived",
                                headers=auth_headers)
        assert response.status_code == 200
        goals = response.json()
        assert any(g["goalId"] == goal_id for g in goals["goals"])
    ...
    def test_concurrent_activity_logging(self, api_client, auth_headers):
        ...
        response = api_client.get(f"/v1/goals/{goal_id}/activities?activityDate=2024-01-20",
                                headers=auth_headers)
        assert response.status_code == 200
