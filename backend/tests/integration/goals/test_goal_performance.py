...
class TestGoalPerformance:
    ...
    def test_response_time_targets(self, api_client, auth_headers):
        ...
        start = time.time()
        response = api_client.get("/v1/goals", headers=auth_headers)
        list_time = time.time() - start
        assert response.status_code == 200
        assert list_time < 0.2  # 200ms

        # 4. Log activity - target < 200ms
        activity = {"value": 10, "unit": "items", "activityType": "progress"}
    ...
    def test_concurrent_goal_creation(self, api_client):
        ...
        def create_user_goals(user_id: int) -> List[Dict[str, Any]]:
            ...
            for i in range(goals_per_user):
                ...
                start = time.time()
                response = api_client.post("/v1/goals", json=goal_data, headers=headers)
                ...
                results.append(
                    {
                        ...
                        "duration": duration,
                        "goal_id": (
                            response.json().get("goalId") if response.status_code == 201 else None
                        ),
                    }
                )
    ...
    def test_activity_logging_burst(self, api_client, auth_headers):
        ...
        def log_activity(index: int) -> Dict[str, Any]:
            ...
            return {"status": response.status_code, "duration": duration, "index": index}
    ...
    def test_progress_calculation_performance(self, api_client, auth_headers):
        ...
        for period in periods:
            ...
            response = api_client.get(
                f"/v1/goals/{goal_id}/progress?period={period}", headers=auth_headers
            )
            duration = time.time() - start
    ...
    def test_list_goals_with_filters_performance(self, api_client):
        ...
        user_headers = {"Authorization": "Bearer perf-test-token", "X-User-ID": str(uuid.uuid4())}
        ...
        for i in range(100):
            goal_data = {
                ...
                "target": {
                    ...
                    "unit": "items",
                    "period": (
                        "day" if patterns[i % len(patterns)] in ["recurring", "limit"] else None
                    ),
                    "targetDate": (
                        "2024-12-31"
                        if patterns[i % len(patterns)] in ["milestone", "target"]
                        else None
                    ),
                    "direction": "increase",
                },
            ...
            response = api_client.post("/v1/goals", json=goal_data, headers=user_headers)
            assert response.status_code == 201
        ...
        for filters in filter_tests:
            ...
            start = time.time()
            response = api_client.get(f"/v1/goals?{query_string}", headers=user_headers)
            duration = time.time() - start
    ...
    async def test_async_concurrent_operations(self, api_client):
        ...
        async def create_goal(session: aiohttp.ClientSession, user_id: str) -> str:
            """Async create goal."""
            headers = {"Authorization": f"Bearer user-{user_id}-token", "X-User-ID": user_id}
            ...
            async with session.post(
                f"{base_url}/goals", json=goal_data, headers=headers
            ) as response:
                data = await response.json()
                return data["goalId"]

        async def log_activity(
            session: aiohttp.ClientSession, user_id: str, goal_id: str, value: int
        ) -> int:
            """Async log activity."""
            headers = {"Authorization": f"Bearer user-{user_id}-token", "X-User-ID": user_id}

            activity = {"value": value, "unit": "items", "activityType": "progress"}

            async with session.post(
                f"{base_url}/goals/{goal_id}/activities", json=activity, headers=headers
            ) as response:
                return response.status

        async def get_progress(
            session: aiohttp.ClientSession, user_id: str, goal_id: str
        ) -> Dict[str, Any]:
            """Async get progress."""
            headers = {"Authorization": f"Bearer user-{user_id}-token", "X-User-ID": user_id}

            async with session.get(
                f"{base_url}/goals/{goal_id}/progress", headers=headers
            ) as response:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            progress_tasks = [
                get_progress(session, uid, gid) for uid, gid in zip(user_ids, goal_ids)
            ]
            progress_results = await asyncio.gather(*progress_tasks)

            # Verify results
            assert all(status == 201 for status in activity_results)
            assert all(p["progress"]["currentPeriodValue"] == 15 for p in progress_results)
...
class GoalLoadTest(HttpUser):
    ...
    def create_goal(self):
        ...
        with self.client.post(
            "/v1/goals", json=goal_data, headers=self.headers, catch_response=True
        ) as response:
    ...
    def list_goals(self):
        """List user's goals."""
        self.client.get("/v1/goals", headers=self.headers, name="/v1/goals [LIST]")
    ...
    def get_goal_detail(self):
        ...
        if self.goal_ids:
            ...
            self.client.get(
                f"/v1/goals/{goal_id}", headers=self.headers, name="/v1/goals/{goalId} [GET]"
            )
    ...
    def log_activity(self):
        ...
        if self.goal_ids:
            goal_id = self.goal_ids[0]
            activity = {"value": 10, "unit": "reps", "activityType": "progress"}

            self.client.post(
                f"/v1/goals/{goal_id}/activities",
                json=activity,
