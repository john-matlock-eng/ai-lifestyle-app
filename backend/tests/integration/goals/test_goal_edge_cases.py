...
class TestGoalEdgeCases:
    ...
    def test_goal_validation_errors(self, api_client, auth_headers):
        ...
        response = api_client.post("/v1/goals", json={"title": "Test"}, headers=auth_headers)
        ...
        goal_data = {
            ...
            "target": {"metric": "count", "value": 10, "unit": "reps", "direction": "increase"},
        }
    ...
    def test_activity_validation_errors(self, api_client, auth_headers):
        ...
        activity = {"value": 5000, "unit": "steps", "activityType": "invalid_type"}
        ...
        # 4. Negative value
        activity = {"value": -1000, "unit": "steps", "activityType": "progress"}
    ...
    def test_goal_update_restrictions(self, api_client, auth_headers):
        ...
        update_data = {"goalPattern": "milestone"}  # Cannot change pattern
        ...
        response = api_client.put(
            f"/v1/goals/{goal_id}", json={"status": "completed"}, headers=auth_headers
        )
    ...
    def test_pagination_limits(self, api_client, auth_headers):
        ...
        # 2. Test custom limit
        response = api_client.get("/v1/goals?limit=10", headers=auth_headers)
        assert response.status_code == 200
        result = response.json()
        assert len(result["goals"]) == 10

        # 3. Test max limit enforcement
        response = api_client.get("/v1/goals?limit=150", headers=auth_headers)  # Exceeds max of 100
        assert response.status_code == 400

        # 4. Test page navigation
        response = api_client.get("/v1/goals?page=2&limit=10", headers=auth_headers)
        assert response.status_code == 200
        result = response.json()
        assert result["pagination"]["page"] == 2

        # 5. Test beyond last page
        response = api_client.get("/v1/goals?page=100&limit=20", headers=auth_headers)
        assert response.status_code == 200
        result = response.json()
    ...
    def test_concurrent_goal_limits(self, api_client, auth_headers):
        ...
        # Archive one goal
        response = api_client.delete(f"/v1/goals/{created_goals[0]}", headers=auth_headers)
        assert response.status_code == 200
    ...
    def test_special_characters_and_unicode(self, api_client, auth_headers):
        ...
        assert (
            "<script>" not in activity_response["note"]
            or activity_response["note"] == activity["note"]
        )

    def test_authorization_boundaries(self, api_client):
        """Test authorization and access control."""
        # Create two different users
        user1_headers = {"Authorization": "Bearer user1-token", "X-User-ID": str(uuid.uuid4())}

        user2_headers = {"Authorization": "Bearer user2-token", "X-User-ID": str(uuid.uuid4())}
        ...
        response = api_client.post("/v1/goals", json=goal_data, headers=user1_headers)
        assert response.status_code == 201
        goal_id = response.json()["goalId"]

        # 2. User 2 tries to access User 1's goal
        response = api_client.get(f"/v1/goals/{goal_id}", headers=user2_headers)
        assert response.status_code == 404  # Should not reveal existence

        # 3. User 2 tries to update User 1's goal
        response = api_client.put(
            f"/v1/goals/{goal_id}", json={"title": "Hacked!"}, headers=user2_headers
        )
        assert response.status_code == 404

        # 4. User 2 tries to log activity on User 1's goal
        activity = {"value": 1, "unit": "times", "activityType": "completed"}

        response = api_client.post(
            f"/v1/goals/{goal_id}/activities", json=activity, headers=user2_headers
        )
        assert response.status_code == 404

        # 5. User 2 tries to archive User 1's goal
        response = api_client.delete(f"/v1/goals/{goal_id}", headers=user2_headers)
        assert response.status_code == 404
