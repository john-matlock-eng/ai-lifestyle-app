...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            "Unhandled exception in handler wrapper",
            exc_info=True,
...
def _handle_registration(event: Dict[str, Any], context: Any, request_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            for error in e.errors():
                ...
                validation_errors.append(
                    ValidationErrorModel(field=field_path, message=error["msg"])
                )
            ...
            result = {"statusCode": 400, "headers": get_response_headers(), "body": response_body}
            logger.info(
                "Returning validation error response",
                extra={"status_code": 400, "validation_errors_count": len(validation_errors)},
            )
            return result
        ...
        try:
            ...
            logger.error(
                "Failed to create RegistrationService",
                exc_info=True,
                extra={"error_type": type(e).__name__, "error_message": str(e)},
            )
            raise
        ...
        logger.info(
            "Registration successful", extra={"user_id": str(result.userId), "email": result.email}
        )
        ...
        logger.error("Registration error", extra={"error_code": e.error_code, "details": e.details})
        metrics.add_metric(name="RegistrationErrors", unit=MetricUnit.Count, value=1)

        response = ErrorResponse(
            error=e.error_code, message=e.message, details=e.details, request_id=request_id
        )
        ...
        logger.error(
            "Unexpected error", extra={"error": str(e), "traceback": traceback.format_exc()}
        )
