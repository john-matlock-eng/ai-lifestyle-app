...
class GoalsRepository:
    ...
    def get_goal(self, user_id: str, goal_id: str) -> Optional[Goal]:
        ...
        try:
            response = self.table.get_item(
                Key={"pk": self._user_key(user_id), "sk": self._goal_key(goal_id)}
            )
    ...
    def list_user_goals(
        ...
        try:
            if status:
                ...
                query_params = {
                    "KeyConditionExpression": Key("pk").eq(self._user_key(user_id))
    ...
    def get_user_activities_by_date(
        self, user_id: str, date: datetime, limit: int = 100
    ) -> List[GoalActivity]:
        ...
        try:
            ...
            activities = []
            for item in response["Items"]:
                if item.get("EntityType") == "GoalActivity" or item.get("sk", "").startswith(
                    "ACTIVITY#"
                ):
                    # Remove DynamoDB-specific fields
                    for field in ["pk", "sk", "EntityType", "gsi1_pk", "gsi1_sk", "TTL"]:
    ...
    def batch_get_goals(self, user_id: str, goal_ids: List[str]) -> List[Goal]:
        ...
        try:
            keys = [
                {"pk": self._user_key(user_id), "sk": self._goal_key(goal_id)}
