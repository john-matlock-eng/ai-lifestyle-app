...
from .models import (
    Goal, GoalActivity, GoalPattern, Period, ActivityType,
    PeriodHistory, GoalProgress, TrendDirection
)
...
class ProgressCalculator:
    ...
    def calculate_recurring_progress(
        ...
        successful_periods = sum(
            1 for period, data in period_data.items()
            if data['total'] >= goal.target.value
        )
        ...
        for period, data in period_data.items():
            period_history.append(PeriodHistory(
                period=period,
    ...
    def calculate_milestone_progress(
        ...
        percent_complete = min(100, (total / goal.target.value * 100)) if goal.target.value > 0 else 0
    ...
    def calculate_streak_progress(
        ...
        longest_streak = max(longest_streak, current_streak)
        target_streak = int(goal.target.value)
        percent_complete = min(100, (current_streak / target_streak * 100)) if target_streak > 0 else 0
...
class GoalValidator:
    ...
    def validate_activity_for_goal(activity: GoalActivity, goal: Goal) -> List[str]:
        ...
        if activity.unit and activity.unit != goal.target.unit:
            errors.append(f"Activity unit '{activity.unit}' does not match goal unit '{goal.target.unit}'")
