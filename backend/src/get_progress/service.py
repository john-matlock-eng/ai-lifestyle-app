...
class GetProgressService:
    ...
    def get_goal_progress(
        self, user_id: str, goal_id: str, period: str = "current"
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Retrieved progress for goal {goal_id}: {progress.percent_complete}% complete")
    ...
    def _get_activities_for_period(
        self, user_id: str, goal_id: str, period: str
    ) -> List[GoalActivity]:
        ...
        activities = self.repository.get_goal_activities(
            user_id, goal_id, start_date=start_date, limit=1000  # Get more for analysis
        )
    ...
    def _calculate_statistics(
        self, goal: Goal, activities: List[GoalActivity], period: str
    ) -> Dict[str, Any]:
        ...
        stats["completedActivities"] = type_counts.get("completed", 0) + type_counts.get(
            "progress", 0
        )
    ...
    def _generate_insights(
        self, goal: Goal, activities: List[GoalActivity], progress: Any, statistics: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not activities:
            insights["recommendations"].append("Start logging activities to build insights")
            return insights
        ...
        if progress.trend == "improving":
            ...
            insights["successPatterns"].append(
                "Performance is declining - consider adjusting your approach"
            )
    ...
    def _analyze_time_performance(self, activities: List[GoalActivity]) -> Optional[str]:
        ...
        time_averages = {time: sum(values) / len(values) for time, values in time_values.items()}
    ...
    def _analyze_day_performance(self, activities: List[GoalActivity]) -> Optional[str]:
        ...
        day_averages = {day: sum(values) / len(values) for day, values in day_values.items()}
