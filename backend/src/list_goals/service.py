...
from goals_common import Goal, GoalError, GoalPattern, GoalsRepository, GoalStatus
        ...
        try:
            ...
            while True:
                goals_batch, next_key = self.repository.list_user_goals(
                    user_id, limit=100, last_evaluated_key=last_evaluated_key
                )
            ...
            filtered_goals = self._apply_filters(
                all_goals, status_filter, pattern_filter, category_filter
            )
            ...
            logger.info(
                f"Returning {len(page_goals)} goals for user {user_id} (page {page}/{total_pages})"
            )

            # Convert to response format
            return {
                "goals": [goal.model_dump(by_alias=True) for goal in page_goals],
                "pagination": {"page": page, "limit": limit, "total": total, "pages": total_pages},
            }
