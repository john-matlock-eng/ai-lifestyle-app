...
class JournalEntry(BaseModel):
    ...
    word_count: int = Field(
        0, ge=0, description="Word count (client-provided for encrypted content)"
    )
    ...
    # Goal Integration
    linked_goal_ids: List[str] = Field(
        default_factory=list, description="Goals linked to this entry"
    )
    goal_progress: List[GoalProgress] = Field(
        default_factory=list, description="Goal progress updates"
    )
    ...
    encryption_iv: Optional[str] = Field(
        None, description="Initialization vector for encryption (base64)"
    )
    shared_with: List[str] = Field(
        default_factory=list, description="User IDs this entry is shared with"
    )
...
class CreateJournalEntryRequest(BaseModel):
    ...
    word_count: Optional[int] = Field(
        None, ge=0, description="Word count (required if content is encrypted)"
    )
