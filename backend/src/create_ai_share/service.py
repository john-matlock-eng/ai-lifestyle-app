        ...
        try:
            ...
            for i in range(0, len(batch_items), 25):
                ...
                if response.get('UnprocessedItems'):
                    logger.warning(f"Unprocessed items in batch write: {response['UnprocessedItems']}")
                    # In production, implement retry logic here
            
            logger.info(f"Created AI analysis request {analysis_request_id} with {len(share_ids)} shares")
        ...
        for item_id in item_ids:
            try:
                ...
                if 'Item' not in response:
                    raise ValueError(f"{item_type.capitalize()} {item_id} not found or access denied")
