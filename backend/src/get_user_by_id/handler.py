...
from .errors import DatabaseError, UnauthorizedError, UserError, UserNotFoundError, ValidationError
...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not user_id:
            error_response = ErrorResponse(
                error="VALIDATION_ERROR", message="Missing userId in path", request_id=request_id
            )
        ...
        user_service = UserService(repository=repository, cognito_client=cognito_client)
        ...
        logger.info(
            ...
            extra={"request_id": request_id, "requested_user_id": user_id},
        )
