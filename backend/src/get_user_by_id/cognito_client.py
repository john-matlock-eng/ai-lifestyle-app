...
from .errors import InvalidTokenError, TokenExpiredError, UnauthorizedError
        ...
        try:
            ...
            if claims.get("token_use") != "access":
                raise InvalidTokenError(
                    message="Invalid token use", details={"token_use": claims.get("token_use")}
                )
            ...
            raise TokenExpiredError(message="Access token has expired")
        except jwt.InvalidTokenError as e:
            logger.error(f"Invalid token: {str(e)}")
            raise InvalidTokenError(message="Invalid access token", details={"error": str(e)})
        except Exception as e:
            logger.error(f"Token verification error: {str(e)}")
            raise InvalidTokenError(message="Failed to verify token", details={"error": str(e)})
