...
class CreateShareRequest(BaseModel):
    ...
    # Time limits
    expires_in_hours: Optional[int] = Field(None, ge=1, le=720, description="Hours until expiration")
...
class RecoverySetupRequest(BaseModel):
    ...
    social_threshold: Optional[int] = Field(None, ge=1, description="Required guardians for recovery")
    security_questions: Optional[List[Dict[str, str]]] = Field(None, description="Q&A pairs")
    
    # Encrypted recovery key
    encrypted_recovery_key: str = Field(..., description="Master key encrypted with recovery method")
