...
class UserRepository:
    ...
    def __init__(self, table_name: str):
        ...
        logger.info("User repository initialized", extra={"table_name": table_name})
        ...
        try:
            ...
            if "Item" not in response:
                ...
                raise UserNotFoundError(
                    f"User with ID {user_id} not found", details={"user_id": user_id}
                )
            ...
            logger.info(
                "User retrieved successfully", extra={"user_id": user_id, "email": user_data.email}
            )
            ...
            if error_code == "ResourceNotFoundException":
                raise DatabaseError(
                    f"Table {self.table_name} not found", details={"table_name": self.table_name}
                )
            else:
                raise DatabaseError(
                    f"Database error: {error_message}", details={"error_code": error_code}
                )
            ...
            raise DatabaseError("An unexpected database error occurred", details={"error": str(e)})
    ...
    def get_user_by_email(self, email: str) -> Optional[DynamoDBUser]:
        ...
        try:
            ...
            raise DatabaseError("Database query failed", details={"error": str(e)})
        except Exception as e:
            logger.exception(
                "Unexpected error while querying by email", extra={"error": str(e), "email": email}
            )
            raise DatabaseError("An unexpected database error occurred", details={"error": str(e)})
