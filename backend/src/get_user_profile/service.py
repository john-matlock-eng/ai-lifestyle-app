...
class UserProfileService:
    ...
    def get_user_profile(self, access_token: str) -> UserProfile:
        ...
        logger.info("Processing get user profile request", extra={"has_token": bool(access_token)})

        try:
            # Verify token and get user ID from Cognito
            user_id = self.cognito_client.verify_token_and_get_user_id(access_token)

            logger.info("Token validated successfully", extra={"user_id": user_id})
            ...
            logger.exception("Unexpected error during profile retrieval", extra={"error": str(e)})
            raise ProfileError(
                "An unexpected error occurred while retrieving profile", details={"error": str(e)}
            )

    @tracer.capture_method
    def enrich_profile_with_cognito_data(
        ...
        try:
            ...
            cognito_email_verified = (
                cognito_data["attributes"].get("email_verified", "false") == "true"
            )
            ...
            logger.warning("Failed to enrich profile with Cognito data", extra={"error": str(e)})
