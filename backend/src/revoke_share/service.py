    ...
    def revoke_share(self, user_id: str, share_id: str) -> None:
        ...
        try:
            self.table.update_item(
                Key={"pk": f"SHARE#{share_id}", "sk": f"SHARE#{share_id}"},
        ...
        try:
            ...
            if response.get("Item"):
                ...
                if action == "remove" and recipient_id in shared_with:
                    ...
                    logger.info(
                        f"Updated {item_type} {item_id} shares list - removed {recipient_id}"
                    )
                else:
                    logger.info(
                        f"Recipient {recipient_id} not in shared list for {item_type} {item_id}"
                    )
