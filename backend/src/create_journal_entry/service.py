...
from journal_common import CreateJournalEntryRequest, JournalEntry, JournalRepository, JournalStats
...
class CreateJournalEntryService:
    ...
    def _check_entry_quota(self, user_id: str) -> None:
        ...
        entries, _ = self.repository.list_user_entries(user_id, limit=1)
        ...
        if stats.total_entries >= MAX_JOURNAL_ENTRIES:
            ...
            raise ValueError(
                f"Entry quota exceeded. Maximum {MAX_JOURNAL_ENTRIES} entries allowed."
            )
        ...
        try:
            ...
            if not template_found and entry.template:
                ...
                stats.template_usage.append(
                    TemplateUsage(template=entry.template, count=1, last_used=entry.created_at)
                )
            ...
            if not stats.last_entry_date:
                ...
                if days_since_last <= 7:
                    stats.entries_this_week = (
                        stats.entries_this_week + 1 if stats.entries_this_week else 1
                    )
                else:
                    stats.entries_this_week = 1
                ...
                if days_since_last <= 30:
                    stats.entries_this_month = (
                        stats.entries_this_month + 1 if stats.entries_this_month else 1
                    )
                else:
                    stats.entries_this_month = 1
