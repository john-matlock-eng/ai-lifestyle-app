...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            metrics.add_metric(
                name="UnauthorizedJournalDeletionAttempts", unit=MetricUnit.Count, value=1
            )
        ...
        if not entry_id:
            ...
            metrics.add_metric(
                name="InvalidJournalDeletionRequests", unit=MetricUnit.Count, value=1
            )
        ...
        try:
            ...
            if success:
                # Add metrics
                metrics.add_metric(
                    name="JournalEntryDeletionAttempts", unit=MetricUnit.Count, value=1
                )
                metrics.add_metric(
                    name="SuccessfulJournalEntryDeletions", unit=MetricUnit.Count, value=1
                )

                return {"statusCode": 204, "headers": {"X-Request-ID": request_id}}
            else:
                # Entry not found
                metrics.add_metric(
                    name="JournalEntryNotFoundForDeletion", unit=MetricUnit.Count, value=1
                )
