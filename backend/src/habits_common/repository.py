...
from .errors import HabitNotFoundError, HabitConflictError
...
class HabitRepository:
    ...
    def check_in_habit(self, user_id: str, habit_id: str, check_date: date,
                      completed: bool, note: Optional[str] = None,
                      value: Optional[float] = None) -> HabitCheckInResponse:
        ...
        try:
            ...
            if completed:
                ...
                elif habit.current_streak == 0 or (habit.last_completed and 
                     (check_date - datetime.fromisoformat(habit.last_completed).date()).days > 1):
                ...
                if new_streak in milestones:
                    ...
                    bonus_points = {
                        7: 50, 14: 75, 21: 100, 30: 150,
                        60: 200, 90: 300, 180: 500, 365: 1000
                    }
    ...
    def get_habit_checkin(self, user_id: str, habit_id: str, 
                         check_date: date) -> Optional[HabitCheckInResponse]:
        """Get a specific check-in for a habit."""
        try:
            response = self.table.get_item(
                Key=self._get_checkin_key(user_id, habit_id, check_date)
            )
    ...
    def get_habit_analytics(self, user_id: str, habit_id: str, 
                          period: str = 'month') -> HabitAnalyticsResponse:
        ...
        try:
            ...
            if mid_point > 0:
                ...
                second_half_rate = sum(1 for c in checkins[mid_point:] if c.completed) / (len(checkins) - mid_point)
    ...
    def _update_habit_streak(self, user_id: str, habit_id: str, 
                           new_streak: int, last_completed: date) -> None:
