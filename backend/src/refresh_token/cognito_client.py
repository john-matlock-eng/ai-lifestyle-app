    ...
    def refresh_access_token(self, refresh_token: str) -> CognitoTokenResponse:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"has_secret": bool(self.client_secret), "token_length": len(refresh_token)},
            )
            ...
            if not auth_result.get("AccessToken"):
                logger.error("No access token in Cognito response", extra={"response": response})
            ...
            if error_code == "NotAuthorizedException":
                if "Refresh Token has expired" in error_message:
                    ...
                    raise RevokedTokenError(
                        "The refresh token has been revoked.", details={"cognito_error": error_code}
                    )
                ...
                raise InvalidTokenError(
                    "Invalid refresh token format.", details={"cognito_error": error_code}
                )
            ...
            raise CognitoError(
                "An unexpected error occurred during token refresh", details={"error": str(e)}
            )
    ...
    def get_user_from_access_token(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if error_code in ["NotAuthorizedException", "TokenExpiredException"]:
                raise InvalidTokenError(
                    "Invalid or expired access token", details={"cognito_error": error_code}
                )
            else:
                raise CognitoError(
