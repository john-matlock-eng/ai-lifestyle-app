...
class CognitoClient:
    ...
    def authenticate_user(self, email: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.client.initiate_auth(
                ClientId=self.client_id, AuthFlow="USER_PASSWORD_AUTH", AuthParameters=auth_params
            )
            ...
            if "ChallengeName" in response:
                if response["ChallengeName"] in ["SMS_MFA", "SOFTWARE_TOKEN_MFA"]:
                    ...
                    raise MfaRequiredError(
                        session_token=response["Session"], challenge_name=response["ChallengeName"]
                    )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "tokens_are_same": auth_result.get("AccessToken")
                    == auth_result.get("RefreshToken"),
                    "refresh_equals_id": auth_result.get("RefreshToken")
                    == auth_result.get("IdToken"),
                    "access_equals_id": auth_result.get("AccessToken")
                    == auth_result.get("IdToken"),
                },
            )
            ...
            logger.error(
                ...
                extra={"error_code": error_code, "error_message": error_message, "email": email},
            )
            ...
            if error_code == "NotAuthorizedException":
                ...
                raise CognitoError(
                    "Invalid parameters provided", error_code, {"message": error_message}
                )
            else:
                raise CognitoError(error_message, error_code)
    ...
    def get_user_attributes(self, access_token: str) -> Dict[str, str]:
        ...
        try:
            ...
            raise CognitoError(
                "Failed to get user attributes", error_code, {"message": error_message}
            )
    ...
    def update_failed_login_attempts(self, email: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to update login attempts counter", extra={"email": email, "error": str(e)}
            )
    ...
    def reset_failed_login_attempts(self, email: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to reset login attempts counter", extra={"email": email, "error": str(e)}
            )
