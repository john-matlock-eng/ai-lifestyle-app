...
from .errors import InvalidCredentialsError, LoginError, MfaRequiredError, UserNotFoundError
from .models import CognitoAuthResponse, LoginRequest, LoginResponse, MfaLoginResponse, UserProfile
...
class LoginService:
    ...
    def __init__(self, user_repository: UserRepository, cognito_client: CognitoClient):
        """
    ...
    def login_user(
        self, request: LoginRequest, ip_address: Optional[str] = None
        ...
        try:
            ...
            try:
                ...
                logger.info("MFA required for login", extra={"email": request.email})
                return MfaLoginResponse(
                    sessionToken=e.details["session_token"], mfaRequired=True, tokenType="Bearer"
                )
            ...
            if not user_data:
                logger.error(
                    "User authenticated but not found in database", extra={"email": request.email}
                )
                raise UserNotFoundError(request.email)

            # Update successful login
            self.user_repository.record_login_attempt(
                email=request.email, success=True, ip_address=ip_address
            )

            # Update last login timestamp
            self.user_repository.update_last_login(
                user_id=user_data["userId"], login_timestamp=datetime.utcnow()
            )
            ...
            logger.error(
                ...
                extra={"email": request.email, "error": str(e), "error_type": type(e).__name__},
            )
            raise LoginError(
                "An error occurred during login. Please try again.", {"original_error": str(e)}
            )
